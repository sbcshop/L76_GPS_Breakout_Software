...
void L76Uart_SendString(char *data)
{
  uint16_t i;
  for(i=0; data[i] != '\0'; i++){
...
void L76Uart_ReceiveString(char *data, uint16_t Num)
{  
  uint16_t i;
  while(1){
    if(mySerial.available()){
...
void Set_GPIOMode(uint16_t Pin, uint16_t mode)
{
...
/******************************************************************************
function: GCJ-02 international standard converted to Baidu map BD-09 standard
******************************************************************************/
...
/******************************************************************************
function: GPS's WGS-84 standard is converted into GCJ-02 international standard
******************************************************************************/
...
void L76K_Send_Command(char *data)
{
    ...
    L76Uart_SendString(data);
    mySerial.write('*');
    L76Uart_SendString(Check_char);
    mySerial.write('\r');
    mySerial.write('\n');
}

/******************************************************************************
function: Analyze GNRMC data in L76K, latitude and longitude, time
******************************************************************************/
GNRMC L76K_Gat_GNRMC()
{
    ...
    L76Uart_ReceiveString(buff_t, BUFFSIZE);
     Serial.print(buff_t);
    add = 0; 
    while(add < BUFFSIZE){
...
/******************************************************************************
function: Convert GPS latitude and longitude into Baidu map coordinates
******************************************************************************/
Coordinates L76K_Baidu_Coordinates()
{
    Coordinates temp;
...
******************************************************************************/
Coordinates L76K_Google_Coordinates()
{
    Coordinates temp;
